...
# Make triton optional for platforms that don't support it (e.g., macOS)
try:
    import triton
...
def edt_cpu(data: torch.Tensor) -> torch.Tensor:
    """
    CPU implementation of EDT using scipy.
    ...
    try:
        ...
        raise ImportError(
            "scipy is required for CPU EDT computation. "
    ...
    output_np = torch.zeros_like(data, dtype=torch.float32).cpu().numpy()
    
    # Compute EDT for each image in the batch
    for i in range(B):
        # distance_transform_edt expects 0 for background, non-zero for foreground
        # Our data has the opposite convention, so we invert it
        output_np[i] = distance_transform_edt(data_np[i] == 0)
    
    # Convert back to torch tensor
    return torch.from_numpy(output_np).to(data.device)
...
def edt_triton(data: torch.Tensor):
    ...
    - CUDA: Uses optimized Triton kernel
    - MPS/CPU: Uses scipy-based implementation
    ...
    # Check if we can use the CUDA Triton kernel
    if data.is_cuda:
        ...
        # Use CPU implementation for MPS and CPU devices
        return edt_cpu(data)
